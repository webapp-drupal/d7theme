<?php
  ...
  $paywall_plan_items = field_get_items('node', $node, 'field_paywall_plan_names'); 
  $paywall_plan_names = call_user_func_array('array_merge', $paywall_plan_items); 
  // echo 'here: <pre>' . print_r($paywall_plan_names, true) . '</pre>';
  ...
  if($paywall_plan_names){
    if( isset( $paywall_plan_names['field_paywall_print_plan']['und'][0]['value'] ) ){
      $custom['paywall_print_plan'] = $paywall_plan_names['field_paywall_print_plan']['und'][0]['value'];
      $product_count++;
    }
    if( isset( $paywall_plan_names['field_paywall_bundle_plan']['und'][0]['value'] ) ){
      $custom['paywall_bundle_plan'] = $paywall_plan_names['field_paywall_bundle_plan']['und'][0]['value'];
      $product_count++;
    }
    if( isset( $paywall_plan_names['field_paywall_digital_plan']['und'][0]['value'] ) ){
      $custom['paywall_digital_plan'] = $paywall_plan_names['field_paywall_digital_plan']['und'][0]['value'];
      $product_count++;
    }
  ...
  if( isset( field_get_items('node', $node, 'field_subs_show_delivery_country')[0]['value']) ){
    $custom['show_delivery_country'] = (boolean) field_get_items('node', $node, 'field_subs_show_delivery_country')[0]['value'];
  }
...
<?php elseif( isset($_GET['id']) && isset( $_GET['productName']) && isset( $_GET['productName']) && isset( $_GET['productTitle'] ) && (isset( $_GET['payment-plans'] ) && !empty( $_GET['payment-plans'] ) )  ): ?>
  ...
  <?php
    ...
    $sanitized_payment_plans = htmlspecialchars( filter_var( $_GET['payment-plans'] , FILTER_SANITIZE_URL) );
  ?>
  ...
  <div class="step-1-2">
    <p>
      <ev-product-selection id="<?php echo $sanitized_product_id; ?>" 
                           ...
                           show-hidden="true"
                           <?php if( $custom['show_delivery_country'] && $sanitized_product_name != 'Digital_only' ) echo 'show-delivery-country="true" default-country="United Kingdom"'; ?>
                           <?php
...
<?php else: ?>
  ...
  <section class="subscription-options" id="start">
    ...
    <div class="row">
      <?php if( isset($custom['paywall_print_plan']) && $custom['paywall_print_plan'] ){ ?>
      <div class="large-<?php echo ($product_count == 3) ? '4' : '6'; ?> columns <?php echo ($product_count == 1) ? 'col-center' : ''; ?>">
        <div class="subscription-option print-subscription">
          <h2>Print</h2>
          <div class="product-image">
          </div>
          <div class="cta"><a href="?id=<?php echo variable_get('paywall_PM_adult_print_id') ?>&productName=<?php echo variable_get('paywall_PM_adult_print_name') ?>&productTitle=<?php echo urlencode('Print Only'); ?>&payment-plans=<?php echo @$custom['paywall_print_plan']; ?>"><strong><?php echo @$custom['subscribe_button_text']; ?></strong></a></div>
    ...
    <?php } ?>
      ...
      <?php if( isset($custom['paywall_bundle_plan']) && $custom['paywall_bundle_plan'] ){ ?>
      <div class="large-<?php echo ($product_count == 3) ? '4' : '6'; ?> columns <?php echo ($product_count == 1) ? 'col-center' : ''; ?>">
        <div class="subscription-option print-digital-subscription best-value">
          ...
          <!-- .app-images -->
        <div class="cta"><a href="?id=<?php echo variable_get('paywall_PM_adult_print_digital_id') ?>&productName=<?php echo variable_get('paywall_PM_adult_print_digital_name') ?>&productTitle=<?php echo urlencode('Digital and Print'); ?>&payment-plans=<?php echo @$custom['paywall_bundle_plan']; ?>"><strong><?php echo @$custom['subscribe_button_text']; ?></strong></a></div>
      ...
      <?php if( isset($custom['paywall_digital_plan']) && $custom['paywall_digital_plan'] ){ ?>
      <div class="large-<?php echo ($product_count == 3) ? '4' : '6'; ?> columns <?php echo ($product_count == 1) ? 'col-center' : ''; ?>">
        <div class="subscription-option digital-subscription">
          ...
          <!-- .app-images -->
          <div class="cta"><a href="?id=<?php echo variable_get('paywall_PM_adult_digital_id') ?>&productName=<?php echo variable_get('paywall_PM_adult_digital_name') ?>&productTitle=<?php echo urlencode('Digital Only'); ?>&payment-plans=<?php echo @$custom['paywall_digital_plan']; ?>"><strong><?php echo @$custom['subscribe_button_text']; ?></strong></a></div>
